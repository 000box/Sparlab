...
class TekkenGameReader:
    def __init__(self):
        ...
        self.MemoryAddressOffsets = {'player_data_pointer_offset': 0x0344E320, 'p2_data_offset': 0x6930, 'rollback_frame_offset': 0x1AB10,
                                    'player_data_second_pointer_offset': 0, 'p2_end_block_offset': 0xD0, 'movelist_size': 2000000, 'expected_module_address': 0x140000000}
        self.GameDataAddress = {'frame_count': 0x1A4D0, 'facing': 0xAD4, 'timer_in_frames': 0x1A158}
        self.EndBlockPlayerDataAddress = {'round_wins': 0x1A16C, 'p2_wins': 0x19BB4, 'display_combo_counter': 0x1A200, 'display_combo_damage': 0x1A1D8, 'display_juggle_damage': 0x1A1DC,
                                            'p2_display_combo_counter': 0x19c18, 'p2_display_combo_damage': 0x19c20, 'p2_display_juggle_damage': 0x19c24}
        self.PlayerDataAddress = {'move_timer': 0x1F0, 'move_id': 0x31C, 'recovery': 0x368, 'hit_outcome': 0x3A4, 'attack_type': 0x3DC, 'simple_move_state': 0x3E0, 'stun_type': 0x3E4, 'throw_tech': 0x3F4,
                                    'complex_move_state': 0x408, 'power_crush': 0x4FA, 'jump_flags': 0x594, 'cancel_window': 0x5B8, 'damage_taken': 0x73C, 'input_attack': 0x162C, 'input_direction': 0x1630,
                                    'attack_startup': 0x6920, 'attack_startup_end': 0x6924, 'char_id': 0xD4, 'attack_damage': 0x2FC, 'throw_flag': 0x3F8, 'x': 0xC00, 'y': 0xC04, 'z': 0xC08, 'hitbox1': 0xC0C,
                                    'hitbox2': 0xC10, 'hitbox3': 0xC14, 'hitbox4': 0xC18, 'hitbox5': 0xC1C, 'activebox_x': 0x1060, 'activebox_y': 0x1064, 'activebox_z': 0x1068, 'health_percent': 0x11E8, 'movelist_to_use': 0x1208,
                                    'input_counter': 0x15B8, 'raw_array_start': 0xABC, 'rage_flag': 0x99C, 'mystery_state': 0x534, 'juggle_height': 0x11D8}
        self.NonPlayerDataAddresses = {'P1_Movelist': (0x03419720,0x2E8), 'P2_Movelist': (0x0341C660, 0x2E8), 'OPPONENT_NAME': (0x03403C40, 0x0, 0x8, 0x114), 'OPPONENT_SIDE': (0x03403C40, 0x0, 0x8, 0x70), 'P1_CHAR_SELECT': (0x033B4E68, 0x80, 0x3CC), 'P2_CHAR_SELECT': (0x033B4E68, 0x80, 0x584), 'STAGE_SELECT': (0x033B4E68, 0x80, 0x78), 'Matchlist0_PlayerName': (0x03336410, 0x2C0, 0x138),
                                        'Matchlist0_PING': (0x03336410, 0x2C0, 0x114), 'Matchlist0_CharId': (0x03336410, 0x2C0, 0x180), 'Matchlist0_Rank': (0x03336410, 0x2C0, 0x184), 'Matchlist0_Wins': (0x03336410, 0x2C0, 0x188),
                                         'WARMUP_PLAYER_WINS1':  (0x033B4408, 0x50, -0x34), 'WARMUP_PLAYER_NAME2': (0x033B4408, 0x50, 0x140), 'WARMUP_PLAYER_WINS2': (0x033B4408, 0x50, 0x10C)}
    ...
    def GetUpdatedState(self, rollback_frame = 0):
        ...
        if (self.needReacquireModule):
            ...
            if self.module_address == None:
                ...
                self.ReacquireEverything()
            elif(self.module_address != self.MemoryAddressOffsets['expected_module_address']):
        ...
        if self.module_address != None:
            ...
            try:
                try:
                    a = self.MemoryAddressOffsets['player_data_pointer_offset'] + self.module_address
                except IndexError:
                    a = self.module_address
                player_data_base_address = self.GetValueFromAddress(processHandle, a, is64bit = True)
                if player_data_base_address == 0:
                    ...
                    for i in range(8):  # for rollback purposes, there are 8 copies of the game state, each one updatating once every 8 frames
                        try:
                            potential_second_address = second_address_base + (i * self.MemoryAddressOffsets['rollback_frame_offset'])
                        except IndexError:
                            potential_second_address = second_address_base
                        try:
                            potential_frame_count = self.GetValueFromAddress(processHandle, potential_second_address + self.GameDataAddress['frame_count'])
                        except IndexError:
                            potential_frame_count = self.GetValueFromAddress(processHandle, potential_second_address)
                    ...
                    player_data_frame = self.GetBlockOfData(processHandle, player_data_second_address, self.MemoryAddressOffsets['rollback_frame_offset'])

                    for data_type, value in self.PlayerDataAddress.items():
                        p1_value = self.GetValueFromDataBlock(player_data_frame, value, 0, self.IsDataAFloat(data_type))
                        p2_value = self.GetValueFromDataBlock(player_data_frame, value, self.MemoryAddressOffsets['p2_data_offset'], self.IsDataAFloat(data_type))
                        p1_bot.player_data_dict['PlayerDataAddress.'+data_type] = p1_value
                        p2_bot.player_data_dict['PlayerDataAddress.'+data_type] = p2_value

                    for data_type, value in self.EndBlockPlayerDataAddress.items():
                        p1_value = self.GetValueFromDataBlock(player_data_frame, value)
                        p2_value = self.GetValueFromDataBlock(player_data_frame, value, self.MemoryAddressOffsets['p2_end_block_offset'])
                    ...
                    bot_facing = self.GetValueFromDataBlock(player_data_frame, self.GameDataAddress['facing'])
                    timer_in_frames = self.GetValueFromDataBlock(player_data_frame, self.GameDataAddress['timer_in_frames'])

                    pda = self.PlayerDataAddress
                    # This is ugly and hacky
                    for axis, startingAddress in ((k, v) for k,v in self.PlayerDataAddress.items() if k in ('x', 'y', 'z')):
                        ...
                        for i in range(23):
                            ...
                            p2_coord_array.append(self.GetValueFromDataBlock(player_data_frame, startingAddress + (i * positionOffset), self.MemoryAddressOffsets['p2_data_offset'], is_float=True))
    ...
    def PopulateMovelists(self, processHandle, data_type):
        movelist_str = self.NonPlayerDataAddresses[data_type]
        # movelist_trail = list(map(to_hex, movelist_str.split()))
        movelist_trail = movelist_str
        movelist_address = self.GetValueFromAddress(processHandle, self.module_address + movelist_trail[0], is64bit=True)
        movelist_block = self.GetBlockOfData(processHandle, movelist_address, self.MemoryAddressOffsets["movelist_size"])
