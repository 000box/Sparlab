...
class XInputJoystick(event.EventDispatcher):
    ...
    def dispatch_axis_events(self, state):
        ...
        lax = axis_fields['l_thumb_x']
        oldlax = getattr(self._last_state.gamepad, 'l_thumb_x')
        newlax = getattr(state.gamepad, 'l_thumb_x')
        lax_datasize = ctypes.sizeof(lax)
        t_oldlax = self.translate(oldlax, lax_datasize)
        t_newlax = self.translate(newlax, lax_datasize)

        lay = axis_fields['l_thumb_y']
        oldlay = getattr(self._last_state.gamepad, 'l_thumb_y')
        newlay = getattr(state.gamepad, 'l_thumb_y')
        lay_datasize = ctypes.sizeof(lay)
        t_oldlay = self.translate(oldlay, lay_datasize)
        t_newlay = self.translate(newlay, lay_datasize)

        olapos = (t_oldlax,t_oldlay)
        nlapos = (t_newlax,t_newlay)
        ...
        rax = axis_fields['r_thumb_x']
        oldrax = getattr(self._last_state.gamepad, 'r_thumb_x')
        newrax = getattr(state.gamepad, 'r_thumb_x')
        rax_datasize = ctypes.sizeof(rax)
        t_oldrax = self.translate(oldrax, rax_datasize)
        t_newrax = self.translate(newrax, rax_datasize)

        ray = axis_fields['r_thumb_y']
        oldray = getattr(self._last_state.gamepad, 'r_thumb_y')
        newray = getattr(state.gamepad, 'r_thumb_y')
        ray_datasize = ctypes.sizeof(ray)
        t_oldray = self.translate(oldray, ray_datasize)
        t_newray = self.translate(newray, ray_datasize)

        orapos = (t_oldrax,t_oldray)
        nrapos = (t_newrax,t_newray)

        if nrapos != orapos and ((t_newrax > 0.08 or t_newrax < -0.08) or (t_newray > 0.08 or t_newray < -0.08)) and \
            (abs(t_oldrax - t_newrax) > 0.00000000500000000 or abs(t_oldray - t_newray) > 0.000000005000000000):
            self.dispatch_event('on_axis', 'ra', nrapos)

        rt = axis_fields['right_trigger']
        oldrt = getattr(self._last_state.gamepad, 'right_trigger')
        newrt = getattr(state.gamepad, 'right_trigger')
        rt_datasize = ctypes.sizeof(rt)
        t_oldrt = self.translate(oldrt, rt_datasize)
        t_newrt = self.translate(newrt, rt_datasize)

        if t_newrt != t_oldrt and abs(t_oldrt - t_newrt) > 0.00000000500000000 and (t_newrt == 0 or t_newrt == 1):
            self.dispatch_event('on_axis', 'rt', t_newrt)

        lt = axis_fields['left_trigger']
        oldlt = getattr(self._last_state.gamepad, 'left_trigger')
        newlt = getattr(state.gamepad, 'left_trigger')
        lt_datasize = ctypes.sizeof(lt)
        t_oldlt = self.translate(oldlt, lt_datasize)
        t_newlt = self.translate(newlt, lt_datasize)

        if t_newlt != t_oldlt and abs(t_oldlt - t_newlt) > 0.00000000500000000 and (t_newlt == 0 or t_newlt == 1):
            self.dispatch_event('on_axis', 'lt', t_newlt)
